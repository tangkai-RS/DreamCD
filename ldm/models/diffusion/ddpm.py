...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if self.noise_cond:
            # cond_betas = get_noise_cond_beta_schedule(timesteps)
            ...
            self.gammas = get_noise_cond_gamma_schedule(timesteps, channels)
...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c, t=None):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                if isinstance(c, dict):
                    c_out = dict()
                    for k, v in c.items():
                        c = self.cond_stage_model.encode(v)
                        c_out[k] = [c]
                    c = c_out
                else:
        ...
        if self.noise_cond: # TODO: mod by tk, noise condition
            ...
            # else:
            c = self._add_cond_noise(c)    
        return c
        # return dict(c_crossattn=c, c_concat=c)
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox']:
                    ...
                    xc = batch
                elif cond_key == "segmentation_segmentation":
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(c, t=t)
    ...
    def _add_cond_noise(self, cond):
        noise = torch.randn_like(cond)
        noise = noise / torch.sqrt(torch.tensor(cond.shape[1]).to(cond.device))
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
               ...
               mask=None, x0=None, shape=None, change=False, **kwargs):
        if shape is None:
        ...
        return self.p_sample_loop(cond,
                                  ...
                                  mask=mask, x0=x0, change=change)

    @torch.no_grad() # TODO: the core of sampling, mod by tk
    def sample_log(self, cond, batch_size,ddim, ddim_steps,
                   change_background=False, label_A_ds=None, label_B_ds=None,
                   content_correlation_scale=0.6, with_adain=False,
                   x0_style=None, **kwargs):
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log['conditioning'] = xc
            elif isimage(xc): # c == 3 image
                log["conditioning"] = xc
            if ismap(xc): # segmentation label 
